...
import type { RefereeActionState } from "./actions";
...
export default function RefereeForm({ referee, onClose }: RefereeFormProps) {
  const isEdit = !!referee;
  const action = isEdit ? updateReferee : createReferee;
  const [state, formAction, isPending] = useActionState(action, initialState);
  const formRef = useRef<HTMLFormElement>(null);
  const [photoUrl, setPhotoUrl] = useState<string | null>(
    referee?.photoUrl ?? null,
  );
      ...
      <div className="mx-4 max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-lg border border-border bg-background p-6 shadow-xl">
        <div className="mb-6 flex items-center justify-between">
          <h2 className="text-lg font-bold uppercase tracking-wider text-foreground">
            {isEdit ? "Modifier l'arbitre" : "Ajouter un arbitre"}
          </h2>
          <button
            onClick={onClose}
        ...
        <form ref={formRef} action={formAction} className="space-y-4">
          {isEdit && <input type="hidden" name="id" value={referee.id} />}
