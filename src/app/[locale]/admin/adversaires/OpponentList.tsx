...
import { deleteOpponent } from "./actions";
...
export default function OpponentList({
  ...
  const [editOpponent, setEditOpponent] = useState<OpponentData | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<OpponentData | null>(null);
  ...
  function handleEdit(opponent: OpponentData) {
  ...
  function handleConfirmDelete() {
    ...
    startTransition(async () => {
      const result = await deleteOpponent(deleteTarget.id);
      if (result.error) {
  ...
  return (
    <div>
      ...
      <p className="mb-4 text-sm text-muted-foreground">
        {filtered.length} adversaire{filtered.length > 1 ? "s" : ""}
        {filtered.length !== opponents.length && ` sur ${opponents.length}`}
      </p>
              ...
              {filtered.map((opponent) => (
                ...
                >
                  ...
                  <td className="hidden px-4 py-3 text-muted-foreground md:table-cell">
                    {opponent.venue ? opponent.venue.name : "—"}
                  </td>
                  <td className="hidden px-4 py-3 text-muted-foreground lg:table-cell">
                    {opponent._count.matches}
                  </td>
                  <td className="px-4 py-3">
