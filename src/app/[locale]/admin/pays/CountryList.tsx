...
export default function CountryList({ countries }: CountryListProps) {
  ...
  const [editCountry, setEditCountry] = useState<CountryData | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<CountryData | null>(null);
  ...
  function handleEdit(country: CountryData) {
  ...
  return (
    <div>
      ...
      <p className="mb-4 text-sm text-muted-foreground">
        {filtered.length} pays{filtered.length !== countries.length && ` sur ${countries.length}`}
      </p>
      ...
      {/* Modal formulaire */}
      {showForm && (
        <CountryForm country={editCountry} onClose={handleCloseForm} />
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="mx-4 w-full max-w-sm rounded-lg border border-border bg-background p-6 shadow-xl">
            ...
            <p className="mb-4 text-sm text-muted-foreground">
              ...
              ({deleteTarget.code}) ? Cette action est irréversible.
            </p>
