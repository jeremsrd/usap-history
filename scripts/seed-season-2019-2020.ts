/**
 * Crée les vingt-trois matchs de la saison 2019-2020, celle que le Covid a
 * arrêtée.
 *
 * Le championnat s'interrompt après la 23ᵉ journée, en mars 2020, et ne
 * reprend pas : ni barrages, ni demi-finales, ni finale, et la LNR ne publie
 * rien au-delà. L'USAP est alors **deuxième avec 76 points**, à un seul point
 * de Colomiers — 16 victoires, aucun nul, 7 défaites, 671 points marqués pour
 * 426 encaissés. Le classement officiel sert de garde-fou aux agrégats, et
 * l'arithmétique tombe juste : 16×4 + 12 points de bonus font 76.
 *
 * Deuxième saison de Pro D2 reprise, après 2020-2021 : `utiliserDivision`
 * bascule `lnr.ts` sur `prod2.lnr.fr`, et les seize clubs sont déjà en base,
 * ce sont les mêmes que l'année suivante.
 *
 * Source unique, la LNR. Pas de campagne européenne — l'USAP jouait en
 * deuxième division.
 *
 ...
 *
 * Usage :
 *   npx tsx scripts/seed-season-2019-2020.ts --dry
 *   npx tsx scripts/seed-season-2019-2020.ts
 *
...
/**
 * Aucun club à créer : les seize de 2019-2020 sont ceux de 2020-2021, tous
 * entrés en base avec cette saison-là.
 */
...
async function championnat(echecs: string[]): Promise<Rencontre[]> {
  ...
  // Aucune phase finale : le Covid a arrêté le championnat après la 23ᵉ
  // journée, et la LNR ne publie ni barrages, ni demi-finales, ni finale.
...
async function cloreLaSaison(seasonId: string, startYear: number) {
  ...
  const agregats = {
    ...
    // Deuxième au moment de l'arrêt, à un point de Colomiers.
    finalRanking: 2,
  };
  ...
  const OFFICIEL = { wins: 16, draws: 0, losses: 7, pointsFor: 671, pointsAgainst: 426, points: 76 };
  const ecarts = [
