/**
 * Crée les vingt-six matchs de la saison 2011-2012.
 *
 * Vingt-six journées et rien après : l'USAP termine **onzième avec 49
 * points** — 9 victoires, 2 nuls, 15 défaites, 515 points marqués pour 578
 * encaissés, 9 points de bonus. Trois ans après le titre de 2009 et deux ans
 * après la finale de 2010, le club s'installe dans le ventre mou ; il
 * descendra en 2014.
 *
 * LE CLASSEMENT ADDITIONNE LES PHASES FINALES, comme ceux des saisons
 * voisines : les six premiers y comptent 27, 28 ou 29 journées. **L'USAP, elle,
 * est bien à 26** — onzième, elle n'a joué aucun barrage —, si bien que sa
 ...
 * récentes. La campagne européenne de 2011-2012 reste donc hors base, comme
 * celles de 2012-2013, 2013-2014 et 2018-2019.
 *
 ...
 * Colombes pour le Racing, Charléty pour le Stade Français, Vénissieux pour
 * Lyon (cf. `lib/stades.ts`).
 *
 * Usage :
 *   npx tsx scripts/seed-season-2011-2012.ts --dry
 *   npx tsx scripts/seed-season-2011-2012.ts
 *
...
/**
 * Aucun club à créer : les treize adversaires de Top 14 de 2011-2012 sont tous
...
async function cloreLaSaison(seasonId: string, startYear: number) {
  ...
  // journées.
  const OFFICIEL = {
    wins: 9, draws: 2, losses: 15, pointsFor: 515, pointsAgainst: 578,
    points: 49, bonus: 9,
  };
  const ecarts = [
  ...
  if (!DRY_RUN) {
    await prisma.season.update({
      where: { id: seasonId },
      // Onzième : ni championne, ni promue, ni reléguée.
