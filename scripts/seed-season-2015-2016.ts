/**
 * Crée les trente matchs de la saison 2015-2016, la deuxième de Pro D2 après
 * la relégation de 2014.
 *
 * Trente journées, et rien après : l'USAP termine **septième avec 73 points**
 * — 15 victoires, 1 nul, 14 défaites, 676 points marqués pour 615 encaissés,
 * 11 points de bonus —, à deux places de la phase finale. Le classement
 * officiel de la LNR sert de garde-fou aux agrégats, et l'arithmétique tombe
 * juste : 15×4 + 1×2 + 11 font 73.
 *
 * PAS DE PHASE FINALE POUR L'USAP, comme en 2016-2017 et pour la même raison :
 * la Pro D2 ne qualifiait alors que **quatre** clubs, le premier montant
 * directement. Lyon est monté en tant que premier ; Bayonne (2e), Aurillac
 * (3e), Mont-de-Marsan (4e) et Colomiers (5e) se sont disputé la seconde
 * place en demi-finales, et Bayonne l'a emportée. Il n'y a donc ni barrage,
 * ni demi-finale, ni finale à lire pour Perpignan, septième.
 *
 * PARTICULARITÉ DU CLASSEMENT DE LA LNR, la même qu'en 2016-2017 et
 * 2017-2018 : la page ne donne qu'un **total de bonus**, 11, sans séparer
 * l'offensif du défensif. Le script contrôle donc la somme `BO + BD`, et non
 * chaque compteur — c'est tout ce que la source permet d'affirmer.
 *
 * Source unique, la LNR, sur `prod2.lnr.fr` : pas de campagne européenne,
 * l'USAP jouait en deuxième division. Ce site archive bien 2015-2016 — les
 * vingt-trois de chaque camp, les faits, les changements et l'arbitre sont
 * publiés sur les trente feuilles.
 *
 ...
 * la démonstration ; il n'y a rien à faire ici, mais c'est bon à savoir si un
 * total refusait de retomber.
 *
 ...
 *
 * Un seul club manquait à la base, **Tarbes**, descendu depuis et que l'USAP
 * n'a plus croisé. Comme Albi et Bourgoin, la LNR ne lui connaît plus que ce
 * nom court : ni le classement, ni le calendrier, ni les feuilles de match
 * n'écrivent autre chose que « Tarbes ». C'est donc ce que la base porte,
 * plutôt qu'un nom complet tiré de mémoire. Son écusson est le même bouclier
 * gris que les deux autres (cf. `fetch-club-logos.ts`).
 *
 * Le lieu se déduit du camp — Aimé-Giral à domicile, le stade de l'adversaire
 * à l'extérieur —, sans exception cette saison : aucune rencontre sur terrain
 * neutre, faute de phase finale.
 *
 * Usage :
 *   npx tsx scripts/seed-season-2015-2016.ts --dry
 *   npx tsx scripts/seed-season-2015-2016.ts
 *
...
/**
 * Le seul club de Pro D2 que la base ne connaît pas encore. Le nom est celui
 * du classement officiel de la LNR, relevé sur `prod2.lnr.fr/classement`, et
 * non de mémoire — et la LNR n'en écrit pas d'autre, ce club ayant quitté le
 * championnat et sa page avec.
 */
const NOUVEAUX_ADVERSAIRES = [
  { name: "Tarbes", shortName: "Tarbes", city: "Tarbes", pays: "FR" },
];
...
/** Les trente journées de Pro D2, depuis la LNR. */
async function championnat(echecs: string[]): Promise<Rencontre[]> {
  ...
  for (const phase of phases) {
    ...
    // Un coup d'envoi à 00:00 veut dire « heure inconnue » : sans ce détour,
    // le Perpignan-Dax de la J1 reculerait au 20 août (cf. `momentDuMatch`).
...
async function cloreLaSaison(seasonId: string, startYear: number) {
  ...
  const agregats = {
    ...
    // Septième, à deux places de la phase finale.
    finalRanking: 7,
  };
  ...
  // Comme en 2016-2017, la page de classement ne sépare pas BO et BD : elle
  // n'affiche qu'un total de 11. On contrôle donc la somme, seul chiffre que
  // la source permet d'affirmer.
  const OFFICIEL = {
    wins: 15, draws: 1, losses: 14, pointsFor: 676, pointsAgainst: 615,
    points: 73, bonus: 11,
  };
  const ecarts = [
  ...
  if (!DRY_RUN) {
    await prisma.season.update({
      where: { id: seasonId },
      // Ni championne, ni promue, ni reléguée : septième de Pro D2.
...
async function main() {
  ...
  for (const r of rencontres) {
    ...
    // Trente journées de phase régulière : aucun match couperet cette saison.
