/**
 * Crée les vingt-six matchs de la saison 2012-2013, la dernière saison pleine
 * de l'USAP en Top 14 avant la relégation.
 *
 * Vingt-six journées et rien après : l'USAP termine **septième avec 61
 * points** — 13 victoires, aucun nul, 13 défaites, 593 points marqués pour 607
 * encaissés, 9 points de bonus —, à une place de la phase finale. Elle
 * descendra la saison suivante.
 *
 * LE CLASSEMENT ADDITIONNE LES PHASES FINALES, comme ceux de 2013-2014 et de
 * 2014-2015 : les six premiers y comptent 27, 28 ou 29 journées. **L'USAP,
 * elle, est bien à 26** — septième, elle n'a joué aucun barrage —, si bien que
 * sa ligne est de la phase régulière pure et se reprend telle quelle.
 *
 ...
 * d'écart, bonus défensif à sept points. Les deux bornes de ce barème sont
 * attestées par les saisons voisines (cf. CLAUDE.md, « Points de bonus »).
 *
 ...
 * récentes. La campagne européenne de 2012-2013 reste donc hors base, comme
 * celles de 2013-2014 et de 2018-2019.
 *
 ...
 *
 * LE LIEU SE DÉDUIT DU CAMP, et cette déduction vieillit mal. Elle rattache
 * chaque déplacement au stade **d'aujourd'hui** du club visité, ce qui devient
 * faux à mesure qu'on remonte : le Racing 92 est en base au Paris La Défense
 * Arena, ouvert en 2017, et jouait alors à Colombes. C'est la limite connue de
 * `Opponent.venueId`, qui ne porte qu'un terrain par club et ignore le temps ;
 * elle vaut pour toutes les saisons anciennes déjà reprises, pas seulement
 * pour celle-ci.
 *
 * Usage :
 *   npx tsx scripts/seed-season-2012-2013.ts --dry
 *   npx tsx scripts/seed-season-2012-2013.ts
 *
...
/**
 * Aucun club à créer : les treize adversaires de Top 14 de 2012-2013 sont tous
...
async function cloreLaSaison(seasonId: string, startYear: number) {
  ...
  const agregats = {
    ...
    // Septième, à une place de la phase finale.
    finalRanking: 7,
  };
  ...
  // journées.
  const OFFICIEL = {
    wins: 13, draws: 0, losses: 13, pointsFor: 593, pointsAgainst: 607,
    points: 61, bonus: 9,
  };
  const ecarts = [
  ...
  if (!DRY_RUN) {
    await prisma.season.update({
      where: { id: seasonId },
      // Septième : ni championne, ni promue, ni reléguée — la relégation
      // viendra la saison suivante.
