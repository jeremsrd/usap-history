/**
 * Crée les vingt-six matchs de la saison 2013-2014, celle de la relégation en
 * Pro D2.
 *
 * Vingt-six journées et rien après : l'USAP termine **treizième sur quatorze
 * avec 51 points** — 10 victoires, 1 nul, 15 défaites, 486 points marqués pour
 * 593 encaissés, 9 points de bonus — et descend, avec Biarritz, dernier. C'est
 * la saison qui ouvre les quatre années de Pro D2 déjà en base, 2014-2015 à
 * 2017-2018.
 *
 * **PREMIÈRE SAISON DE TOP 14 REPRISE EN REMONTANT**, et c'est un changement
 * de site : la LNR sépare ses deux divisions, et tout se lit ici sur
 * `top14.lnr.fr`. Le script n'appelle donc pas `utiliserDivision`, `top14`
 * étant la valeur par défaut du module. Ses archives de 2013-2014 sont
 * complètes — les vingt-trois de chaque camp, l'arbitre, les faits et les
 * changements sont publiés sur les vingt-six feuilles.
 *
 * LE CLASSEMENT ADDITIONNE LES PHASES FINALES, comme celui de 2014-2015 : les
 * six premiers y comptent 27, 28 ou 29 journées. **L'USAP, elle, est bien à
 * 26** — treizième, elle n'a joué aucun barrage —, si bien que sa ligne est de
 * la phase régulière pure et se reprend telle quelle. Le piège ne mord pas
 * ici, mais il est là.
 *
 * PARTICULARITÉ COMMUNE AUX SAISONS ANCIENNES : la page ne donne qu'un
 * **total de bonus**, 9, sans séparer l'offensif du défensif. Le script
 * contrôle donc la somme `BO + BD`.
 *
 * ET CE TOTAL EST UN TEST, le symétrique de celui de 2014-2015. 2013-2014 est
 * la **dernière** saison où le bonus défensif s'obtient à sept points d'écart
 * et non cinq (cf. CLAUDE.md, « Points de bonus »), et deux défaites le
 * démontrent : 23-30 contre Clermont à la treizième journée et 12-19 au Stade
 * Français à la quinzième, toutes deux à exactement sept points, toutes deux
 * avec bonus. Sous la règle de 2014-2015 le compte tomberait à 7 et le script
 * refuserait d'écrire.
 *
 ...
 * récentes. La campagne européenne de 2013-2014 reste donc hors base, comme
 * celle de 2018-2019, tant qu'aucune source officielle ne la rouvre.
 *
 ...
 *
 * Le lieu se déduit du camp, sans exception : aucune rencontre sur terrain
 * neutre, faute de phase finale.
 *
 * Usage :
 *   npx tsx scripts/seed-season-2013-2014.ts --dry
 *   npx tsx scripts/seed-season-2013-2014.ts
 *
...
/**
 * Aucun club à créer : les treize adversaires de Top 14 de 2013-2014 sont tous
...
async function cloreLaSaison(seasonId: string, startYear: number) {
  ...
  const agregats = {
    ...
    // Treizième sur quatorze, reléguée en Pro D2.
    finalRanking: 13,
  };
  ...
  // d'affirmer — et ce total teste au passage la borne de 2013-2014, dernière
  // saison où le bonus défensif s'obtient à sept points d'écart et non cinq.
  //
  ...
  // journées.
  const OFFICIEL = {
    wins: 10, draws: 1, losses: 15, pointsFor: 486, pointsAgainst: 593,
    points: 51, bonus: 9,
  };
  const ecarts = [
  ...
  if (!DRY_RUN) {
    await prisma.season.update({
      where: { id: seasonId },
      // Treizième sur quatorze : reléguée en Pro D2, où elle passera quatre
      // saisons avant de remonter en 2018.
      data: { ...agregats, totalPoints: points, champion: false, promoted: false, relegated: true },
    });
  }
}
