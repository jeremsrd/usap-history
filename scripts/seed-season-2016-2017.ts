/**
 * Crée les trente matchs de la saison 2016-2017, la deuxième saison de Pro D2
 * après la relégation de 2014.
 *
 * Trente journées, et rien après : l'USAP termine **sixième avec 79 points** —
 * 16 victoires, 1 nul, 13 défaites, 744 points marqués pour 589 encaissés, 13
 * points de bonus —, à une place de la phase finale. Le classement officiel de
 * la LNR sert de garde-fou aux agrégats, et l'arithmétique tombe juste :
 * 16×4 + 1×2 + 13 font 79.
 *
 * PAS DE PHASE FINALE POUR L'USAP, et ce n'est pas un oubli. Cette saison-là,
 * la Pro D2 ne qualifiait que **quatre** clubs : Montauban (3e) et
 * Mont-de-Marsan (4e) « qualifiés d'office » en demi-finale face à Agen (2e)
 * et Biarritz (5e), Oyonnax montant directement en tant que premier. Il n'y a
 * donc ni barrage, ni demi-finale, ni finale à lire pour Perpignan : le
 * calendrier de la LNR le confirme, ses deux demi-finales et sa finale ne
 * mettent en jeu que ces quatre clubs.
 *
 * PARTICULARITÉ DU CLASSEMENT DE LA LNR, la même qu'en 2017-2018 : la page ne
 * donne qu'un **total de bonus**, 13, sans séparer l'offensif du défensif. Le
 * script contrôle donc la somme `BO + BD`, et non chaque compteur — c'est tout
 * ce que la source permet d'affirmer.
 *
 * Source unique, la LNR, sur `prod2.lnr.fr` : pas de campagne européenne,
 * l'USAP jouait en deuxième division. Ce site archive bien 2016-2017 — les
 ...
 *
 * Deux clubs manquaient à la base — Albi et Bourgoin, descendus depuis et que
 * l'USAP n'a plus croisés. **La LNR ne leur connaît plus que ce nom court** :
 * leurs pages de club ont disparu avec eux, et ni le classement, ni le
 * calendrier, ni les feuilles de match n'écrivent autre chose qu'« Albi » et
 * « Bourgoin ». C'est donc ce que la base porte, plutôt qu'un nom complet tiré
 * de mémoire.
 *
 ...
 *
 * Usage :
 *   npx tsx scripts/seed-season-2016-2017.ts --dry
 *   npx tsx scripts/seed-season-2016-2017.ts
 *
...
/**
 * Les deux clubs de Pro D2 que la base ne connaît pas encore. Les noms sont
 * ceux du classement officiel de la LNR, relevé sur `prod2.lnr.fr/classement`,
 * et non de mémoire — et la LNR n'en écrit pas d'autre, ces deux clubs ayant
 * quitté le championnat et leur page avec.
 */
const NOUVEAUX_ADVERSAIRES = [
  { name: "Albi", shortName: "Albi", city: "Albi", pays: "FR" },
  { name: "Bourgoin", shortName: "Bourgoin", city: "Bourgoin-Jallieu", pays: "FR" },
];
...
async function cloreLaSaison(seasonId: string, startYear: number) {
  ...
  const agregats = {
    ...
    // Sixième, à une place de la phase finale.
    finalRanking: 6,
  };
  ...
  // Comme en 2017-2018, la page de classement ne sépare pas BO et BD : elle
  // n'affiche qu'un total de 13. On contrôle donc la somme, seul chiffre que
  // la source permet d'affirmer.
  const OFFICIEL = {
    wins: 16, draws: 1, losses: 13, pointsFor: 744, pointsAgainst: 589,
    points: 79, bonus: 13,
  };
  const ecarts = [
  ...
  if (!DRY_RUN) {
    await prisma.season.update({
      where: { id: seasonId },
      // Ni championne, ni promue, ni reléguée : sixième de Pro D2.
